                                    ...
                                    <div class="card-body" style="overflow: scroll;">
                                        <table id="example23"
                                            ...
                                            <tbody>
                                                <?php
                                                $i = 1;
                                                $query = "SELECT * FROM partnership_tax ORDER BY par_id DESC";
                                                $result = mysqli_query($con, $query);
                                                while ($row = mysqli_fetch_array($result)) {
                                                    $c_id = $row["c_id"];
                                                    $customer = mysqli_fetch_assoc(mysqli_query($con, "SELECT * FROM `user` WHERE `c_id`=\"$c_id\" AND `user_type`=2"));
                                                ?>
                                                    <tr>

                                                        <td><?= $i++; ?></td>
                                                        <td><?= $customer["f_name"]; ?><br><?= $customer["mobile"]; ?><br><?= $customer["email"]; ?></td>
                                                        <td><?= $customer["partnership_name"] ?></td>
                                                        <td><?= $customer["partnership_tin_num"] ?></td>
