                                ...
                                <div class="card">
                                    ...
                                    <table id="example23"
                                            ...
                                            <tbody>
                                                <?php
                                                $i = 1;
                                                $query = "SELECT * FROM corporate_tax ORDER BY cor_id DESC";
                                                $result = mysqli_query($con, $query);
                                                while ($row = mysqli_fetch_array($result)) {
                                                    $c_id = $row["c_id"];
                                                    $customer = mysqli_fetch_assoc(mysqli_query($con, "SELECT * FROM `user` WHERE `c_id`=\"$c_id\" AND `user_type`=3"));
                                                ?>
                                                    <tr>

                                                        <td><?= $i++; ?></td>
                                                        <td><?= $customer["f_name"]; ?><br><?= $customer["mobile"]; ?><br><?= $customer["email"]; ?></td>
                                                        <td><?= $customer["corporate_name"] ?></td>
                                                        <td><?= $customer["corporate_tin_num"] ?></td>
                                                        <td><?= $customer["reg_date"] ?></td>
                                                        <td><?= $row["t_year"]; ?></td>
                                                        <td><?php
                                                            if ($row["sector"] == 1) {
                                                                echo 'Genaral Corporate';
                                                            } else if($row["sector"] == 2){
                                                                echo 'Liker & Wine';
                                                            } else if($row["sector"] == 3){
                                                                echo 'Forign Services';
                                                            }?>
                                                        </td>
                                                        <td><?= number_format($row["bis_profit"], 2, '.', ','); ?></td>
